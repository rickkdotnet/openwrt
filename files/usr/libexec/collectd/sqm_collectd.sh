...
# rickk - used for nss offloaded fq_codel 
handle_nss() {
        ifc="$1"

        # Overall
        json_get_vars bytes drops backlog overlimits requeues packets

        echo "PUTVAL \"$HOSTNAME/sqm-$ifc/qdisc_bytes\" interval=$INTERVAL N:$bytes"
        echo "PUTVAL \"$HOSTNAME/sqm-$ifc/qdisc_drops\" interval=$INTERVAL N:$drops"
        echo "PUTVAL \"$HOSTNAME/sqm-$ifc/qdisc_backlog\" interval=$INTERVAL N:$backlog"
        echo "PUTVAL \"$HOSTNAME/sqm-$ifc/qdisc_overlimits\" interval=$INTERVAL N:$overlimits"
        echo "PUTVAL \"$HOSTNAME/sqm-$ifc/qdisc_requeues\" interval=$INTERVAL N:$requeues"
        echo "PUTVAL \"$HOSTNAME/sqm-$ifc/qdisc_packets\" interval=$INTERVAL N:$packets"
}
...
process_qdisc() {
	...
	# rickk: nss drivers produce garbled json, strip it out
	if [[ "$jsn" == *"nssfq_codel"* ]]; then
		jsn=$(echo $jsn | sed 's/accel_mode 0// g' | sed 's/target[^"]*//' | sed 's/ maxpacket.*/}}/' | sed 's/new_flows_len.*//')
	fi
